...
function makeLibSignalRepository(auth) {
    ...
    const sessionMutex = (0, make_mutex_1.makeKeyedMutex)();
    const lockAddr = (jid, task) => sessionMutex.mutex(jidToSignalProtocolAddress(jid).toString(), task);
    return {
        decryptGroupMessage({ group, authorJid, msg }) {
            const senderName = jidToSignalSenderKeyName(group, authorJid);
            const cipher = new Group_1.GroupCipher(storage, senderName);
            return sessionMutex.mutex(senderName.toString(), () => cipher.decrypt(msg));
        },
        async processSenderKeyDistributionMessage({ item, authorJid }) {
            ...
            return sessionMutex.mutex(senderNameStr, async () => {
