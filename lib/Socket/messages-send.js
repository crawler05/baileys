...
const makeMessagesSocket = (config) => {
    ...
    const dmSendMutex = (0, make_mutex_1.makeMutex)();
    let mediaConn;
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = false }) => {
        return dmSendMutex.mutex(() => relayMessageUnlocked(jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI }));
    };
    const relayMessageUnlocked = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                const meJids = [];
                const otherJids = [];
                const isSelfChat = !isGroup && !isStatus && user === meUser;
                for (const { user, device } of devices) {
